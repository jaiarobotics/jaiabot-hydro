...
if [ ! -z "$jaiabot_systemd_type" ]; then
    ...
    if [[ "$jaiabot_systemd_type" == *"bot"* ]]; then
        ...
        (export PATH=${HOME}/jaiabot/${build_dir}/bin:$PATH;
         ./systemd-local.sh ${jaiabot_systemd_type} --bot_index $jaia_bot_index --fleet_index $jaia_fleet_index --electronics_stack $jaia_electronics_stack --imu_type $jaia_imu_type --arduino_type $jaia_arduino_type --bot_type ${jaia_bot_type,,} --motor_harness_info_type ${jaia_motor_harness_info_type} $jaia_simulation --enable)
