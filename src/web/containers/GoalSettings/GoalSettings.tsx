...
export class GoalSettingsPanel extends React.Component {
    ...
    handleCoordChange(e: ChangeEvent<HTMLInputElement>, coordType: LatLon) {
        ...
        if (
            value.length < maxCharacterWidth &&
            (value.slice(-1) === "." ||
                (value.slice(-1) === "0" && Number(value) > min && Number(value) < max))
        ) {
            ...
            this.setState({ pauseNumModif: false, enterNegative }, () =>
                this.getCoordValue(coordType),
            );
    ...
    async deleteWaypoint() {
        ...
        if (
            !(await CustomAlert.confirmAsync(
                `Are you sure you want to delete Waypoint ${wptNum}?`,
                "Delete Waypoint",
            ))
        ) {
            return;
    ...
    render() {
        ...
        return (
            <div className="goal-settings-panel-outer-container">
                <div
                    className="goal-settings-panel-inner-container"
                    id="goal-settings-content-panel"
                >
                    ...
                    <div className="goal-settings-wpt-input-container">
                        ...
                        <div
                            className={`goal-settings-delete-wpt-container button-jcc ${!isEditMode ? "goal-settings-hide" : ""}`}
                            onClick={() => this.deleteWaypoint()}
                        >
                    ...
                    <div className="goal-settings-line-break"></div>
                    <div
                        className={`goal-settings-move-container ${!isEditMode ? "goal-settings-hide" : ""}`}
                    >
                    ...
                    <div
                        className={`goal-settings-line-break ${!isEditMode ? "goal-settings-hide" : ""}`}
                    ></div>
                    <div className="goal-settings-label coord-label">Lat:</div>
                    <input
                        className="goal-settings-input coord-input"
                        value={this.getCoordValue(LatLon.LAT)}
                        onChange={(e) => this.handleCoordChange(e, LatLon.LAT)}
                        disabled={!isEditMode}
                    />
                    <div className="goal-settings-label coord-label">Lon:</div>
                    <input
                        className="goal-settings-input coord-input"
                        value={this.getCoordValue(LatLon.LON)}
                        onChange={(e) => this.handleCoordChange(e, LatLon.LON)}
                        disabled={!isEditMode}
                    />
                    <div className="goal-settings-line-break"></div>
                    <div className="goal-settings-label task-label" id="task-scroll-hook">
                        Task:
                    </div>
                    <TaskSettingsPanel
                        task={goal?.task}
                ...
                <div
                    className={`goal-settings-button-container ${!isEditMode ? "goal-settings-single-button-container" : ""}`}
                >
                    <button
                        className={`goal-settings-btn ${!isEditMode ? "goal-settings-hide" : ""}`}
                        onClick={this.cancelClicked.bind(this)}
                    >
                        Cancel
                    </button>
                    <button
                        className={`goal-settings-btn ${!isEditMode ? "goal-settings-single-btn" : ""}`}
                        onClick={this.doneClicked.bind(this)}
                    >
                        {!isEditMode ? "Close" : "Done"}
                    </button>
                </div>
            </div>
        );
