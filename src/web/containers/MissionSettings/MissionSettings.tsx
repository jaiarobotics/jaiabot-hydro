...
export class MissionSettingsPanel extends React.Component {
            ...
            <div className="mission-settings-panel-outer-container">
                ...
                <div className="mission-settings-panel-container">
                    ...
                    <FormControl sx={{ minWidth: 120 }} size="small">
                        <Select
                            onChange={(evt: SelectChangeEvent) =>
                                this.handleRallyFeatureSelection(evt, true)
                            }
                            value={this.props.startRally?.get("num") ?? ""}
                        >
                            {this.getSortedRallyFeatures().map((rallyFeature) => {
                                if (rallyFeature.get("num") !== this.props.endRally?.get("num")) {
                                    return (
                                        <MenuItem value={rallyFeature.get("num")}>
                                            {rallyFeature.get("num")}
                                        </MenuItem>
                                    );
                                }
                            })}
                    ...
                    <FormControl sx={{ minWidth: 120 }} size="small">
                        <Select
                            onChange={(evt: SelectChangeEvent) =>
                                this.handleRallyFeatureSelection(evt, false)
                            }
                            value={this.props.endRally?.get("num") ?? ""}
                        >
                            {this.getSortedRallyFeatures().map((rallyFeature) => {
                    ...
                    >
                        ...
                        <TaskSettingsPanel
                            ...
                            enableEcho={this.props.enableEcho}
                            onChange={(missionStartTask) => {
                                this.setState({ missionStartTask });
                            }}
                        />
                    </div>
                    ...
                    <div
                        className={`mission-settings-srp-container 
                ...
                <div className="mission-settings-button-container">
                    <button className="mission-settings-btn" onClick={() => this.props.onClose()}>
                        Cancel
                    </button>
                    <button
                        className={`mission-settings-btn ${!this.props.missionPlanningGrid ? "disabled" : ""}`}
                        onClick={this.applyMissionClicked.bind(this)}
                        disabled={!this.props.missionPlanningGrid}
                    >
                        Apply
                    </button>
                </div>
    ...
    async applyMissionClicked() {
        if (
            this.props.areThereRuns() &&
            !(await CustomAlert.confirmAsync(
                "Adding this new mission will delete the current mission. Are you sure?",
        ...
        this.props.onMissionApply?.(
            this.props.startRally,
            this.props.endRally,
            this.state.missionStartTask,
            this.state.missionEndTask,
        );
    }
